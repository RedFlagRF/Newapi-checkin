...
- ✅ 错误处理和超时控制
- ✅ 支持手动触发和定时任务
- ✅ **多通道推送通知**（Qmsg 酱 / PushPlus / Server 酱 / 企业微信，签到完成后自动推送结果）
...
底层复用通用 push 模块（位于 `push/` 目录），按通道拆分独立环境变量，配哪个用哪个，可同时配置多个：

| 通道 | 环境变量 | 申请地址 |
|------|----------|----------|
| Qmsg 酱（QQ） | `PUSH_QMSG_KEY` | https://qmsg.zendee.cn |
| PushPlus（微信） | `PUSH_PUSHPLUS_TOKEN` | https://www.pushplus.plus |
| Server 酱（微信） | `PUSH_SERVER_KEY` | https://sct.ftqq.com |
| 企业微信群机器人 | `PUSH_WORK_WECHAT_ROBOT_KEY` | 群设置 → 群机器人 → webhook key |
| 企业微信内部应用 | `PUSH_WORK_WECHAT_CORPID` + `PUSH_WORK_WECHAT_CORP_SECRET` + `PUSH_WORK_WECHAT_AGENT_ID`（三项齐全才生效） | 企业微信管理后台 |

#### 1. 申请通道 Key

挑选你常用的通道，按其官网指引申请 Key。

#### 2. 添加 GitHub Secrets

在仓库 Settings → Secrets and variables → Actions 中添加对应 Secret（名字与上表一致）。例如只用 Qmsg 酱：

| Secret 名称 | 值 |
|-------------|-----|
| `PUSH_QMSG_KEY` | 你的 Qmsg Key |
